    ...
    <div className={"overflow-hidden max-h-80"}>
      ...
      >
        <div
          ...
          className={`rounded-[16px] h-[80%] w-full ${
            isHovered ? "lg:h-[80%]" : "lg:h-full"
          } p-4 flex flex-col gap-4 transition-all duration-500 ease-in-out`}
        >
      ...
      <div
        className={`relative bottom-16 p-1 opacity-100 ${
          isHovered
            ? "lg:opacity-100 lg:duration-1000"
            : "lg:opacity-0 lg:duration-200"
        } md:transition-all`}
        onMouseOver={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}
      >
