...
const JobSchema = z.object({
  ...
  salaryRange: z.string().min(1, "Salary range is required").refine((value) => {
    if (!value) return true; // pass validation if the value is empty

    const rangeParts = value.split("-").map(part =>
      parseInt(part.replace(/\D/g, ''), 10)
    );

    if (rangeParts.length !== 2) return false; // invalid format

    const [from, to] = rangeParts;
    return from <= to; // validation passes if 'from' is less than or equal to 'to'
  }, {
    message: "Invalid salary range: 'from' value should not be greater than 'to' value."
  }),
  level: JobLevel,
  type: JobType,
  workModel: WorkModel,
